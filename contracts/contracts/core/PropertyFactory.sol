...
contract PropertyFactory is Ownable, ReentrancyGuard {
    ...
    // Map finalized propertyId back to appId for lifecycle updates
    mapping(uint256 => uint256) public propertyToAppId;
    ...
    ft.transferOwnership(marketplaceAddr);
        ...
        a.token = token;
    // Link property to application for future lifecycle updates
    ...
    /// @notice Admin marks a previously finalized application as Removed when its property is deactivated/removed.
    /// Accepts propertyId to avoid the UI having to reverse-lookup the appId.
